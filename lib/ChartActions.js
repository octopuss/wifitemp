...
var ChartActions = {
    ...
    assemblyChartData:function(chart){
        ...
        dataset.data=dataset.data.map(function(n){return n.toFixed(ChartEnums[ChartConstants.READING_PRECISION]);});
    ...
    getDayPlotData: function(chart, sensorId) {
        ...
        for(var i=0;i<entries.length;i++) {
           var entry = entries[i];
            var id  =entry._id;
            if(id.sensorId==sensorId) {
                returnDataSet[id.hour]=entry.avg;
            }
        }
        return this.removeTailingZeroes(returnDataSet);
    },
    //filterZeroValues:function(element) {
    //return element!=0;
    //},
    ...
    getMonthPlotData: function(chart, sensorId) {
        ...
        for(i=0;i<entries.length;i++) {
            ...
            var id  =entry._id;
            if(id.sensorId==sensorId) {
                returnDataSet[id.day-1]=entry.avg;
            }
        }
        return this.removeTailingZeroes(returnDataSet);
    ...
    getYearPlotData: function(chart, sensorId) {
        ...
        for(i=0;i<entries.length;i++) {
            ...
            var id  =entry._id;
            if(id.sensorId==sensorId) {
                returnDataSet[id.month-1]=entry.avg;
            }
        }
        return this.removeTailingZeroes(returnDataSet);
    },
    ...
    getMinMaxAvg: function(chart){
        if(chart.data.minMaxAvgDTO.length!=0) {
            ...
            for (i = 0; i < chart.data.minMaxAvgDTO.length; i++) {
                sensorIds.push(chart.data.minMaxAvgDTO[i]._id.sensorId);
            ...
            for (j = 0; j < sensorIds.length; j++) {
                ...
                for (var k = 0; k < chart.data.minMaxAvgDTO.length; k++) {
                    ...
                    if (dto._id.sensorId == sensorIds[j]) {
    ...
    getPlotData: function(chart, sensorId){
        switch(chart.meta.chartType) {
            ...
            case(ChartConstants.CHART_TYPE_YEAR):
                return this.getYearPlotData(chart,sensorId);

        }
    },
    removeTailingZeroes: function(arr) {
        var popped = false;
